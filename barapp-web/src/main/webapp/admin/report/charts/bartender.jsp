...
<% } %>
	...
	Sytle: <select name="daily_average_chart">
		<% for (int j = 0; j <  de.laures.cewolf.taglib.ChartTypes.typeNames.length; j++) {  %>
			<% if ( de.laures.cewolf.taglib.ChartTypes.typeNames[j].toLowerCase().indexOf("verticalbar") > -1 ) { %>
				<option value="<%= j %>"
				<% if ( j == chart ) { %>selected<% } %>
				><%= de.laures.cewolf.taglib.ChartTypes.typeNames[j] %>		
			<% } %>
		<% } %>		
	</select>	
	</div>
	...
	Bartender: 
		<%
		Map m = ReportUtil.getBartenders();
		%>
...
<% 
DailyAverage da = new DailyAverage();
%>
...
<cewolf:chart
	id="daily_average"
	title="<%= title + from + to %>"
	type="<%= de.laures.cewolf.taglib.ChartTypes.typeNames[chart] %>"
...
<cewolf:img chartid="daily_average" renderer="/cewolf" width="<%= width %>" height="<%= height %>">
	<cewolf:map tooltipgeneratorid="dailyAverage"/>
</cewolf:img>
</div>