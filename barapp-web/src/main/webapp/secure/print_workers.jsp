		...
		<% ShiftWorker[] workers = (ShiftWorker[])cashout.getShift().getShiftWorkers().toArray(new ShiftWorker[cashout.getShift().getShiftWorkers().size()]); %>
			<% for ( int i = 0; i < workers.length; i++ ) { %>
				<tr>
					...
					style="padding-left:10px; padding-right:10px"><nobr><%= workers[i].getBarappUser().getLastname() %>, <%= workers[i].getBarappUser().getFirstname().substring(0,1) %></nobr></td><td align="center" 
